...
echo "== [3/3] DIAGNOSE: FF-types + async/latch cellen =="
RD=""; for f in $GENVHDL; do RD="$RD read_rtlil $f;"; done
{
  echo "$RD"
  echo "read_verilog -sv -DECP5 ${EXTRA_DEFINES:-} $VLOG;"
  echo "hierarchy -top top;"
  echo "proc; opt_clean;"
  echo "tee -o gen/diag_stat.txt stat;"
  echo 'select -list t:$dffsr t:$dffsre t:$aldff t:$aldffe t:$dlatch t:$dlatchsr t:$sr t:$adlatch;'
} > gen/diag.ys
yosys -s gen/diag.ys 2>&1 | tee gen/diag.log
echo "== DIAGNOSE KLAAR =="
