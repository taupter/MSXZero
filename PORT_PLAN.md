...
## MiSTle integration (post-Beta-1 refactor)
MSXZero already runs on MiSTle hardware — the icepi_carrier and the RP2350 FPGA Companion, the same
board and companion NanoMig and the other MiSTle cores use. What it is not yet is a clean MiSTle
*core*: the platform layer is hand-ported inline (`clocks_ecp5`, `serializer_ecp5`, narrowed
`memory.v`) via scattered `` `ifdef ECP5`` rather than sitting on a shared board wrapper.

Since the carrier is shared, the conversion is worth doing. Plan (after the fork boots on hardware —
refactor a known-good design, not a moving target):
1. Pull the NanoMig top from `MiSTle-Dev/NanoMig` as the concrete blueprint for the carrier wiring
   (PLL, SDRAM, GPDI, SD, companion SPI) — it targets the exact same board.
2. Extract a `board_top` exposing the canonical MiSTle interface (clk/reset, SDRAM, RGB+audio,
...
3. Standardise the companion/OSD to the MiSTle FPGA-Companion menu/config conventions.
