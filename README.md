...
- Flash config clock — **done**: `mspi_sclk` now drives the config CCLK pin through the ECP5 `USRMCLK`
  primitive (routes `1/1`), so the design can boot from flash. Remaining is purely on-hardware bring-up
  (HDMI picture, SDRAM memtest, SD/Nextor boot, companion keyboard) — the board is the last-mile
  validation; nothing here is silicon-tested yet.
