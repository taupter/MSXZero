...
| `clk_54m` (Z80) timing | routes at ~30–36 MHz. But the worst path is the **clock-enabled** T80 CPU (runs at 3.58/5.37 MHz — the FFs advance ~1 in 10–15 base cycles), so it's genuinely **multicycle** and the 53.85 MHz check is far stricter than the CPU actually needs → very likely fine on hardware. Open-source tools just can't *express* multicycle (nextpnr) or map it tighter (abc9 tri-state bug); the real fix rides along with the cartridge work (make `ex_bus` a real IO). HW-confirm pending | 70% |
