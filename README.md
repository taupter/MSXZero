...
| Analog audio out (line-out / MIDI) | the core already mixes PSG / SCC / OPLL to stereo 16-bit and plays it over HDMI (Track 1, works today). Analog line-out needs a physical stage the icepi_carrier does not have (confirmed from its BOM — no codec/DAC/jack): a PWM / sigma-delta DAC + RC filter + a jack (cheap), or an I2S codec. MIDI out needs a UART-style pin + a DIN connector | 0% |
...
- Output: all sources are mixed to stereo 16-bit and embedded in the HDMI stream (HDMI audio), so
  ...
  dedicated-board add — the icepi_carrier has no audio codec/DAC/jack (see Track 2).
